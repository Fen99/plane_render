        ...
        pipeline_(pipeline)
{
    ...
    CHECK_SDL_NOTNULL(window_ = 
        SDL_CreateWindow("Plane", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, pipeline_->ScreenWidth(), pipeline_->ScreenHeight(), 0));
    ...
    CHECK_SDL_NOTNULL(buffer_ = 
        SDL_CreateTexture(renderer_, SDL_PIXELFORMAT_BGRA8888, SDL_TEXTUREACCESS_STREAMING, pipeline_->ScreenWidth(), pipeline_->ScreenHeight()));
